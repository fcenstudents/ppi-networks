...
J = open(cwd + "/A-rattus-biggest-clusters-xdim_.txt", 'r')
for line in J:
...
plt.ylabel("J_{MAX}")
plt.title('J_{MAX} entre d_{i,max}/d_{i+1,max} de RATTUS')
...
plt.ylabel("J_{MAX}")
plt.title('J_{MAX} entre d_{i, max}/nx_{max} de RATTUS')
...
plt.ylabel("J_{MAX}")
plt.title('J_{MAX} entre d_{i, max}/igraph_{max}" de RATTUS')

plt.tight_layout()
plt.savefig(cwd + '/result-biggest-rattus.png', format='png')
