...
J = open(cwd + "/B-rattus-[biggest-1]-clusters-xdim_.txt", 'r')
for line in J:
...
plt.ylabel("J_{MAX-1}")
plt.title('J_{MAX-1} entre d_{i,max-1}/d_{i+1,max-1} de RATTUS')
...
plt.ylabel("J_{MAX-1}")
plt.title('J_{MAX-1} entre d_{i, max-1}/nx_{max-1} de RATTUS')
...
plt.ylabel("J_{MAX-1}")
plt.title('J_{MAX-1} entre d_{i, max-1}/igraph_{max-1}" de RATTUS')

plt.tight_layout()
plt.savefig(cwd + '/result-[biggest-1]-rattus.png', format='png')
