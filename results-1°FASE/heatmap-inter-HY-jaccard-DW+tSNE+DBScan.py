...
for j in range(len(dim)-1):
	fnamei = cwd + "/clustering-deepwalk+tsne/c-dbscan-tsne-w10-80walks-40l-d" + str(dim[j]) + "-inter_H-Y_.txt"
	fnameii = cwd + "/clustering-deepwalk+tsne/c-dbscan-tsne-w10-80walks-40l-d" + str(dim[j+1]) + "-inter_H-Y_.txt"
	lsti = []
	lstii = []
	J = open(fnamei, 'r')   
	for line in J:
	...
	for t in range(1, len(lsti)+1):
		labelsi.append('c'+str(t)+'-d'+str(dim[j]))
	for s in range(1, len(lstii)+1):
		labelsii.append('c'+str(s)+'-d'+str(dim[j+1]))
	...
	plt.title("J: DIM "+str(dim[j])+' vs. '+str(dim[j+1])+" - INTER-HY")


fig.tight_layout()
plt.savefig(cwd + '/result-heatmap-interHY-DW+tSNE+DBScan.png', format='png')
