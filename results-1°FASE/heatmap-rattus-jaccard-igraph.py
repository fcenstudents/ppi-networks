...
### necesito cargar cada uno de esos 21 clusters en una lista (levanto archivo con los 21 clusters)
lst_cd_i = []
...
for j in range(len(dim)):
	...
	plt.title("J: CD igraph y DW+tSNE+DBSCAN (DIM "+str(dim[j])+") - RATTUS")
	#fig.tight_layout()
	#plt.savefig(cwd + '/result-heatmap-interHY-igraph-d'+str(dim[j])+'.png', format='png')

fig.tight_layout()
plt.savefig(cwd + '/result-heatmap-rattus-igraph.png', format='png')
