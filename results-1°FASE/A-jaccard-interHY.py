...
#fname = cwd + "/A-interHY-biggest-clusters-xdim_.txt"
#df = np.loadtxt(fname=fname, comments='#', delimiter=' ', unpack=False, skiprows=0)
#print(np.shape(df))
#J = open(cwd + '/jaccard-interHY-biggest-clusters-xdim.txt', 'w') 
#J.write ()
#J.write('\n')
#J.close()
...
J = open(cwd + "/A-interHY-biggest-clusters-xdim_.txt", 'r')
for line in J:
...
plt.xlabel("d_{i,max}/d_{i+1,max}")
plt.ylabel("J_{MAX}")
plt.title('J_{MAX} entre d_{i,max}/d_{i+1,max} de INTER_H-Y')


# comparo clusters más grandes tras aplicar community detection directa a la red (mismo algritmo, versión NetworkX vs igraph)
print(jaccard_similarity(lst[9], lst[10]))

# compara los clusters más grandes de cada dimensión con la versión Networkx
jac_nx = []
...
plt.xlabel("d_{i, max}/nx_{max}")
plt.ylabel("J_{MAX}")
plt.title('J_{MAX} entre d_{i, max}/nx_{max} de INTER_H-Y')
...
plt.xlabel("d_{i, max}/igraph_{max}")
plt.ylabel("J_{MAX}")
plt.title('J_{MAX} entre d_{i, max}/igraph_{max}" de INTER_H-Y')

plt.tight_layout()
plt.savefig(cwd + '/result-biggest-interHY.png', format='png')
